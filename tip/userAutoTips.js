(function(win){
	...
	var DS = {
		inquiry:function(data, str, num){
			if(str == '') return data.slice(0, (num || data.length));
			var i = 0;
			var sd = [];
			while(sd.length < (num || data.length) && i < data.length){
				if(data[i]['value'].indexOf(str)!=-1){
					sd.push(data[i]);
				}
				i++;
			}	
	...
	var AutoTips = function(A){
		...
		_this.start = function(){
	  	...
	  	_this.keyupFn = function(e){
			...
			var chars = val.split("@");
			if(val.indexOf("@")==-1) return _this.hide();
		...
		_this.showList = function(char){
			key = char;
			var friendsData = A["post"].call(this, key);
			var data = DS.inquiry(friendsData, char, A.len);
			...
			var em = '<em>'+ char +'</em>';
			for(var i=0; i<len; i++){
				var hm = data[i]['key'].replace(reg,em);
				h += html.replace(/\$VALUE\$/g, data[i]['value']).replace('$SACCOUNT$', hm).replace('$KEY$', data[i]['key']);
			}
			_this.updatePosstion();
		...
		_this.bind = function(){
			...
			elem.onblur = function(){setTimeout(_this.hide, 200)};
		}
