...
(function($, global){
	...
	Tool.prototype = {
		 ...
		 */
		initColor:function(){
		    var $color = $("#tool-wrap .tool .color").eq(0);
		...
		openAttributePanel:function($attributePanel){
		    var $attributePanels = $('.tool-attribute-panel'); 
		...
		bindEvent:function(){
			...
			$document.delegate('#tool-wrap .tool button', 'click', function(){
				if("Garbage"===this.getAttribute("data-current-tool")) {
					var currentCanvas = global.painter.canvas.currentCanvasContainer.getCanvas();
                	currentCanvas.clear();
                	return ;
				} 
				if("Undo"===this.getAttribute("data-current-tool")) {
					var currentCanvas = global.painter.canvas.currentCanvasContainer.getCanvas();
                	currentCanvas.undo();//撤销上一部
					return ;
				}
				if("Save"===this.getAttribute("data-current-tool")) {
					var currentCanvas = global.painter.canvas.currentCanvasContainer.getCanvas();
	                currentCanvas.save();
	                $.fn.TorangeNotice({type:'success', content:'保存成功'});//弹出提示
					return ;
				}
				if("Import"===this.getAttribute("data-current-tool")) {
					return ;
				}
				if("Export"===this.getAttribute("data-current-tool")) {
				...
				if("Scissors"===this.getAttribute("data-current-tool"))	{
					return ;
				}	    
				var $this = $(this),
