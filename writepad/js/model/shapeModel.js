...
(function($, global){
	...
	Line = function(){
	    ...
	    this.paint = function(context){
            ...
            console.log(option.startX + "moveTo");
            context.lineTo(option.endX, option.endY);
            console.log(option.endX + "lineTo");
    ...
    Arrow = function(){
        ...
        this.paint = function(context){
            ...
            context.moveTo(option.startX, option.startY);
            context.lineTo(option.endX, option.endY);
            context.translate(option.endX, option.endY);  
            //我的箭头本垂直向下，算出直线偏离Y的角，然后旋转 ,rotate是顺时针旋转的，所以加个负号  
            var ang=(option.endX-option.startX)/(option.endY-option.startY);  
            ang=Math.atan(ang);  
            if(option.endY-option.startY >= 0){  
                context.rotate(-ang);  
            }else{  
    ...
    global.painter.model.shapeModel.Arrow = Arrow;
}(jQuery, window));